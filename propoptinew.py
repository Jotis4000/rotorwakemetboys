...
def getData(x,R,B,airfoil,tdist,pitch,cdist,U0,rho,RPM):
    ...
    for i in range(len(x)-1):
        results[i,:] = calcloads.calculate_turbine3tokyodrift(x[i]*R,R,start*R,c[i],U0,RPM/60*2*np.pi,B,airfoil) # dT, dQ, a, a_prime, phi, alpha
...
cdist = sp.interpolate.pchip_interpolate(xi,[0.14570071, 0.16377962,  0.10663266,  0.09252311],x) # for x>0.25
...
plt.plot(x[:(99)],np.degrees(results[:,5]),label=r"$\theta$",color="black")
